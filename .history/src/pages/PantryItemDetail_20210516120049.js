...
function PantryItemDetail({ pantryDetail }) {
	const { name, category,notes,  } = pantryDetail;
	...
	return (
		...
		>
			...
			<>
				<HeaderTitle small>{pantryDetail.name}</HeaderTitle>
				<PantryImageWrapper>
					<img className="img-fluid" src={WholeChicken} alt={pantryDetail.name} />
				</PantryImageWrapper>
				...
				<PantryDetailValuesWrapper>
					<span className="text-center" style={{ color: '#BCBCBC', display: 'flex', alignItems: 'center' }}>
						<Chip
							size="small"
							style={{ color: '#222', background: '#FDC039', marginRight: '2rem' }}
							label={pantryDetail.quantity}
						/>

						<PantryListValues start space>
							<span style={{ fontWeight: '600', textTransform: 'capitalize' }}>{pantryDetail.name}</span>
							<span style={{ color: '#BCBCBC' }}>{pantryDetail.weight}lb</span>
						</PantryListValues>
					</span>
					<PantryDetailTimeWrapper>
						<span>Best Before: {pantryDetail.expiry}</span>
						<span>Created At: {pantryDetail.createdAt}</span>
					</PantryDetailTimeWrapper>
				</PantryDetailValuesWrapper>
				<p style={{ fontWeight: 'bold' }}>Notes</p>
				<p style={{ textTransform: 'capitalize' }}>{pantryDetail.notes}</p>
			</>
